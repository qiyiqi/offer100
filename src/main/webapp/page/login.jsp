...
<html>
  <head>
	...
	<link rel="stylesheet" href="<%=request.getContextPath()%>/common/css/sccl.css">
  ...
  <body class="login-bg">
    <div class="login-box">
        <header>
            <h1>看十条新闻阅读器后台管理系统</h1>
        </header>
        <div class="login-main">
			<form action="/KstBackstage/admin/login" class="layui-form" method="post">
				<input name="__RequestVerificationToken" type="hidden" value="">                
				<div class="layui-form-item">
					<label class="login-icon">
						<i class="layui-icon"></i>
					</label>
					<input type="text" name="aName" lay-verify="userName" autocomplete="off" placeholder="这里输入登录名" class="layui-input">
				</div>
				<div class="layui-form-item">
					<label class="login-icon">
						<i class="layui-icon"></i>
					</label>
					<input type="password" name="aPassword" lay-verify="password" autocomplete="off" placeholder="这里输入密码" class="layui-input">
				</div>
				<div class="layui-form-item">
    ...
    <script>
        layui.use(['layer', 'form'], function () {
            ...
            form.on('submit(login)', function (data) {
				window.location.href = "<%=request.getContextPath()%>/page/index.jsp";
                /*if (errorCount > 5) {
                    layer.open({
